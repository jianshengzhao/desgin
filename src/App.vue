  ...
  <!-- attribute tool -->
    <div class="tool" unselectable="on" onselectstart="return false;">
      <div class="toolBox">
        ...
        <div class="property">
          <label for="">宽 :</label>
          <el-input v-model="inp_w" type='number' :disabled='disabled||wDis' min='0' @change='changeInpW'></el-input>
        </div>
        <div class="property">
          <label for="">高 :</label>
          <el-input v-model="inp_h" type='number' :disabled='disabled||hDis' min='0' @change='changeInpH'></el-input>
        </div>
        <div class="property">
          <label for="">文字 :</label>
          <el-input v-model="inp_size" type='number' :disabled='disabled||editorDisabled' min='12' @change='changeInpSize'></el-input>
        </div>
        <div class="property">
          <label for="">行高 :</label>
          <el-input v-model="inp_line" type='number' :disabled='disabled||editorDisabled' min='12' @change='changeInpLine'></el-input>
        </div>
        <div class="property">
          <label for="">字色 :</label>
  ...
  <!-- dialog弹框 -->
    ...
    <el-dialog
      ...
      <el-row>
        ...
        <el-col :span="3" v-if="linkType == 'news'||linkType == 'coruse'||linkType == 'contactUs'" style="text-align: right">打开方式：</el-col>
        <el-col :span="9" v-if="linkType == 'news'||linkType == 'coruse'||linkType == 'contactUs'">
          <el-select v-model="newsOpenType" placeholder="请选择，默认：新窗口">
            <el-option
      ...
      <el-row v-if="linkType == 'news'">
        ...
        <el-col :span="9">
          <el-cascader
            ...
            v-model="selectNews"
            change-on-select
            @change="handleNewsItemChange">
          </el-cascader>
        </el-col>
      </el-row>
      <el-row v-if="linkType == 'coruse'">
        <el-col :span="3" style="text-align: right">课程来源：</el-col>
        <el-col :span="9">
          <el-select v-model="coruseSource" placeholder="请选择" @change="carryGetCoruseSort">
            <el-option
              key="self"
              label="本校课程"
              value="self">
            ...
            <el-option
              v-for="item in coruseSourceOption"
              :key="item.sourceid"
              :label="item.name"
              :value="item.sourceid">
            </el-option>
        ...
        <el-col :span="9">
          <el-cascader
            ...
            :disabled="coruseSource==''"
            change-on-select
            @change="handleCoruseItemChange">
          </el-cascader>
        </el-col>
      </el-row>
      <span slot="footer" class="dialog-footer">
      ...
      <el-row>
        <el-col class='courseSource'>
          <el-radio-group v-model="radioSource" @change="radioCourseSourceChangeEvent"> <!-- todo -->
            ...
            <el-radio-button v-for="(item, index) in sourceData" :key="item.sourceid" :label="item.sourceid" v-if="false">{{item.name}}</el-radio-button>
          </el-radio-group>
        </el-col>  
      </el-row>  
      <el-row>
      ...
      <el-row class="conCourse">
        <el-col :span="12">
          ...
          <div class="courseList conCourse1"  v-loading="loading">
            <div class="courseLi" v-for="(item, index) in courseListData" :key="item.folderid" :label="item.folderid" :foldername="item.foldername" @click="viewCoursewareEvent(item.folderid, index)">
              <i class="el-icon-circle-check"></i>
              <div class="imgbox">
                <img :src="item.img" >
              </div>
        ...
        <el-col :span="12" class="courseware">
          ...
          <el-input
            ...
            :on-icon-click="searchCoursewareEvent">
          </el-input>
          <div class="cwlist" v-loading="loading">
            <el-radio-group v-model="radioCourseware" @change="radioCoursewareChangeEvent">
              <div class="cwLi" v-for="(item, index) in cwlistData" :key="item.sid">
                ...
                <el-radio-button v-for="(itemcw, index) in item.cwlist" :key="itemcw.cwid" :label="itemcw.cwid" :disabled="itemcw.ism3u8!='1'">{{itemcw.title}}</el-radio-button>
              </div>
            </el-radio-group>
...
<script>
  ...
  export default { // todo: 本地操作保存
      ...
      // ---------------------------------------------
        ...
        // --------------- complete ------------------
          ...
          initialize: function (self, ele) { // 选中模块的执行的操作
            ...
            self.hDis = false
            self.wDis = false
            switch (self.moduleElement.attr('class')) { // todo:特殊模块属性禁用名单
              case 'editor module on_module':
                self.editorDisabled = true
                self.inp_size = ''
                self.inp_line = ''
              break
              case 'contactway module on_module':
                self.editorDisabled = true
                self.inp_size = ''
                self.inp_line = ''
              break
              case 'addcoursetype module on_module':
                self.editorDisabled = true
                self.hDis = true
                self.inp_size = ''
                self.inp_line = ''
              break
              case 'login module on_module':
                self.editorDisabled = true
                self.hDis = true
                self.wDis = true
                self.inp_size = ''
          ...
          bindClickEvent: function (self) { // 画布内选中模块一系列操作事件
            ...
            editBox.on('mousedown', '.resize', function (e) { // 选中小圆点按钮拉伸容器
              ...
              // todo: 拉伸某容器固定显示内容
              editBox.mousemove(function (e) {
                let modClass = self.moduleElement.attr('class')
                switch (modClass) { // todo:特殊模块拉伸禁用名单
                  case 'addcoursetype module on_module':
                    self.hDis = true
                    if (isClass == 'resize e'||isClass =='resize w') {
                      part(e)
                    }
                  break
                  case 'login module on_module':
        ...
        // --------------- todo: ---------------------
          ...
          switchModuleEvent: function (type, onthis, self) { // todo:定位
            let w
            let getParam
            let oa
            switch (type) {
              case 'text':
                ...
                switch (self.linkType) {
                  case 'online':
                    ...
                    if(!oa.attr('href')) return false
                    self.inpOnline = oa.attr('href')
                    break
                  case 'news': 
                    self.getNewsCategorysData()
                    self.newsOpenType = oa.attr('target')
                    if(!oa.attr('selectNews')) return false                    
                    self.selectNews = oa.attr('selectNews').split(',')
                    self.handleNewsItemChange(self.selectNews)
                    break
                  case 'coruse':                    
                    self.getCourseSource()
                    self.newsOpenType = oa.attr('target')
                    if(!oa.attr('selectCoruse')) return false
                    self.coruseSource = oa.attr('corusesource')
                    if (self.coruseSource != 'self') {
                      self.courseSortData(self.coruseSource)
                    } else {
                      self.courseSortData()
                    }
                    self.selectCoruse = oa.attr('selectCoruse').split(',')
                    self.handleCoruseItemChange(self.selectCoruse)
                    break
                  case 'contactUs':
                    self.newsOpenType = oa.attr('target')
                    break
            ...
            // --------------------------------
              ...
              case 'audition':
                ...
                self.radioNextClass = ''                
                self.inp_courseName = ''
                if (onthis.attr('foldername') && onthis.attr('auditioname')) { // 课件定位
                  self.inp_courseName = onthis.attr('foldername')
                  let label = onthis.attr('label')
                  self.searchCourseEvent()
                  self.inp_CoursewareName = onthis.attr('auditioname')
                  self.folderid = label
                  self.searchCoursewareEvent()
                }
                // todoList: 免费试听去无课程来源定位
                if (onthis.attr('auditionid')){
                  self.radioCourseware = onthis.attr('auditionid')
                }
                getParam = {
              ...
              case 'course':
                ...
                self.radioNextClass = ''
                self.inp_courseName = ''
                if (onthis.attr('dataCoruse')) {
                  self.radioSource = onthis.attr('radioSource')
                  self.inp_courseName = onthis.attr('iname')
                  self.searchCourseEvent()
                } else {
                  getParam = {
                    url: '/aroomv3/schsource.html',
      ...
      dialogTextEvent: function () { // 编辑文本窗口
        ...
        switch (self.linkType) {
          ...
          case 'news':
            ...
            let navcm = news[0].split('n')[1]
            let ns
            let itemid
            let url
            if (news.length > 2) {
              itemid = news[2]
              url = '/dyinformation/' + itemid + '.html'
            } else {
              switch (news[0]) {
                case 'news':
            ...
            if (self.newsOpenType) {              
              a.attr('target', self.newsOpenType) 
            }
            a.attr('href', url)            
            a.attr('selectNews', self.selectNews)            
            break
          case 'coruse':
            if (self.coruseSource == '') {
              self.$notify({
                title: '警告',
                message: '请选择课程来源',
                type: 'warning'
            ...
            let coruse = self.selectCoruse
            let folderid
            let curl
            let pid = coruse[0].split('|')[1]
            let sid
            if (coruse.length > 2) {
              folderid = coruse[2]
              curl = '/courseinfo/' + folderid + '.html'
            } else { 
              if (coruse.length == 1) {
                curl = '/platform-1-0-0.html?pid=' + pid 
              } else {
                let type = coruse[1].split('|')[0]
                if (type == 'pid') {
                  curl = '/platform-1-0-0.html?pid=' + pid 
                } else {
                  sid = coruse[1].split('|')[1]
                  curl = '/platform-1-0-0.html?pid=' + pid + '&sid=' + sid
                }
              }
            }
            if (self.newsOpenType) {              
            ...
            a.attr('href', curl)
            a.attr('coruseSource', self.coruseSource)
          ...
          case 'register':
            a.removeAttr('href')
            self.moduleElement.addClass('registerEvent')
            break
          case 'getUserName':
            a.removeAttr('href')
            self.moduleElement.addClass('getUserNameEvent')
            break 
          case 'QQ': 
        ...
        if (self.linkType !== 'register') {
          self.moduleElement.removeClass('registerEvent')
        }
        if (self.linkType !== 'getUserName') {
          self.moduleElement.removeClass('getUserNameEvent')
        }
    ...
    // ------------- 图片基础组件 ------------
        ...
        if (self.coruseSource == 'self') {
          param = {
            ...
            fun: function (response) {
              ...
              for (let i = 0, len = self.selectCoruseOptions.length; i < len; i++) { // 遍历父级分类
                let item = self.selectCoruseOptions[i]
                if (item.pid === pid) { // 筛选到当前父级
                  for (let j = 0, jen = item.children.length; j < jen; j++) {  // 遍历当前父级下的子级分类
                    let jtem = item.children[j]
                    if (jtem.sid === csid) {// 筛选到当前子级
                      let arrFolder = []
          ...
          param = {
            ...
            params: {
              q: '',
              sourceid: self.coruseSource,
              pid: pid,
              sid: sid
            },
            fun: function (response) {
              let itemlist = response.body.data.itemlist
              for (let i = 0, len = self.selectCoruseOptions.length; i < len; i++) { // 遍历父级分类
                let item = self.selectCoruseOptions[i]
                if (item.pid === pid) { // 筛选到当前父级
                  for (let j = 0, jen = item.children.length; j < jen; j++) {  // 遍历当前父级下的子级分类
                    ...
                    if (jtem.sid === csid) {// 筛选到当前子级
                      let arrFolder = []
                      for (let z = 0, zen = itemlist.length; z < zen; z++) {
                        let zitem = itemlist[z]
                        arrFolder.push({
                          label: zitem.iname,
                          value: zitem.itemid
                        })
      ...
      courseSortData: function (val) { // 选择课程 cascader
        let self = this
        let param
        if (val) {
          param = {
            ...
            fun: function (response) {
              let arr = response.body.data.splist
              let obj = []
              for (let i = 0, len = arr.length; i < len; i++) {
                ...
                items.children.push({label: '全部', value: 'pid|' + item.pid, children: [], sid: item.pid})
                if (item.sorts.length) {
      ...
      getCourseSource: function () {
        ...
        let getParam = {
          ...
          fun: function (response) {
            let data = response.body.data
            self.coruseSourceOption =  data
          }
        }
        self.httpget(getParam)
      },
      carryGetCoruseSort: function (val) {
        let self = this
        if (val === 'self') {
          self.courseSortData()
        } else {
          self.courseSortData(val)
        }        
      },
    // ------------- 导航设置 ----------------
      ...
      dialogNavigationEvent: function () { // 导航条设置
        ...
        if (widthT >= 266) {
          ...
          navaBox.css('maxWidth', 890 + widthT + 'px')
        }
                  ...
                  switch (item.navtype) {
                    case '0':
                      if (item.code == 'news') {
                        ...
                        navHtml += '<a href="/contacts.html">' + item.nickname + '</a>'
                      } else {
    ...
    // ------------- 免费试听 ----------------
        ...
        if (value !== 'self') {
          let param = {
            ...
            fun: function (response) {
              ...
              if (!self.moduleElement.attr('dataCoruse')) {
      ...
      radioCoursewareChangeEvent: function (val) {
        let self = this
        let audition = $('.audition')
        let arr = []
        let arro = {}
        ...
        if (self.moduleElement.attr('auditionid') != val){
          if (arro[val]) {
            self.radioCourseware = ''
            self.$notify({
              title: '警告',
              message: '该试听课件已添加过了，请重新选择',
              type: 'warning'
            });
          }
        } else {
          self.radioCourseware = self.moduleElement.attr('auditionid')
        }
      },
      handleSettingAuditionEvent: function () { // todolist:
        let self = this
        let cwData
        if (!self.radioCourseware) {
          ...
          let label = $('.courseOn').attr('label')
          let foldername = $('.courseOn').attr('foldername')
          module.attr('label', label)
          module.attr('foldername', foldername)
          module.attr('auditioname', cwData.title)
    ...
    // ------------- 课程选择 ----------------
      ...
      handleSettingCourseConfirmEvent: function () {
        ...
        if ($('.courseOn').length < 1) {
          ...
          self.moduleElement.attr('radioSource', self.radioSource)
          self.moduleElement.attr('dataCoruse', self.courseitem.itemid)
          self.moduleElement.attr('iname', self.courseitem.iname)
...
/*tool*/
  ...
  .property .el-input.is-disabled .el-input__inner{
    background-color: #fff;
  }
/*leftlibrary*/
  .library{
